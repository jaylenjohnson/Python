...
# noted start of the execution time
t1 = datetime.now()
#slpitting the query by space delimeter
qr_list = query.split()
if "and" in qr_list:
    if "or" in qr_list:
        qr_list.remove("or")
    qr_list.remove("and")
    print "Performing AND search for ", qr_list
    temp = len(qr_list)
    count = 0
    for i, word in enumerate(data_list):
        for inner_word in qr_list:
            if inner_word in word:
                count = count +1   
        if count == temp:
            print "found at ", i, word  
        count = 0
elif ("or" in qr_list) and ("and" not in qr_list):
    qr_list.remove("or")
    print "Performing OR search for ", qr_list
    for i, word in enumerate(data_list):
        for inner_word in qr_list:
            if inner_word in word:
                print "found at ", i, word
                break
elif ("or" not in qr_list) and ("and" not in qr_list):
    print "Performing AND search for ", qr_list
    temp = len(qr_list)
    count = 0
    for i, word in enumerate(data_list):
...
"""
I have taken raw input from User, and splitted it by space delimeter. Which by default is a list, qr_list,
after that I have noted start of execution time. 
"""